...
void process_p1(int fd, struct sockaddr_in clientaddr, socklen_t clientlen) {
        ...
        Rio_writen_w(fd, buf, strlen(buf));    while (1) {
                ...
                if(current_board_index == 10 && winning_player == 0){
                    snprintf(buf, MAXLINE, "Draw!\n");
                    Rio_writen_w(fd, buf, strlen(buf));
                    if(spec_count == 0)
                    {
                            Close(fd);
...
void process_spec(int fd, struct sockaddr_in clientaddr, socklen_t clientlen) {
        ...
        while (1) {
                ...
                while (board_history[board_index] != NULL) {
                        ...
                        if(winning_player != 0) {
                            snprintf(buf, MAXLINE, "Player %d won!\n", winning_player);
                            Rio_writen_w(fd, buf, strlen(buf));
                            Close(fd);
                                exit(0);
                        }
                        ...
                        if (board_index == MAX_BOARD_HISTORY) {
